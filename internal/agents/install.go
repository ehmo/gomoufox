...
func validateOptions(opts Options) error {
	...
	if strings.ContainsAny(opts.Toolset, " \t\r\n\"'") {
		return fmt.Errorf("invalid MCP toolset: %s", opts.Toolset)
	}
...
func mergeMCPJSON(existing []byte, toolset string, extraArgs []string) ([]byte, error) {
	...
	servers["gomoufox"] = map[string]any{
		"command": "gomoufox",
...
func mergeCodexTOML(existing []byte, toolset string, extraArgs []string) ([]byte, error) {
	...
	begin := "# BEGIN gomoufox managed mcp server"
	end := "# END gomoufox managed mcp server"
	block := begin + "\n" +
		"[mcp_servers.gomoufox]\n" +
		"command = \"gomoufox\"\n" +
	...
	if strings.Contains(text, "[mcp_servers.gomoufox]") {
		return nil, errors.New("mcp_servers.gomoufox already exists outside the managed block")
	}
...
func mcpArgs(toolset string, extra []string) []string {
	args := []string{"mcp", "--toolset", toolset}
