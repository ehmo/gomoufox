...
func NewRegistry(defs []Definition) (Registry, error) {
	...
	for _, def := range defs {
		...
		if _, err := parseSemver(def.MinGomoufox); err != nil {
			return Registry{}, fmt.Errorf("invalid min gomoufox version %s: %w", def.MinGomoufox, err)
		}
...
func DefaultRegistry() Registry {
	return mustDefaultRegistry(defaultDefinitions)
}

func mustDefaultRegistry(defs []Definition) Registry {
